<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <div class="flex items-center gap-2 mt-1">
                    ...
                    <span class="text-sm font-bold text-orange-600">Editing: {{ $aboutRole->title }}</span>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto pb-20">
        <form method="POST" action="{{ route('admin.about-roles.update', $aboutRole) }}" class="space-y-8">
            @csrf
            @method('PATCH')
            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                <div class="p-8 md:p-12 space-y-10">
                    ...
                    <div class="space-y-4">
                        ...
                        <x-text-input id="title" name="title" type="text" class="block w-full border-slate-100 bg-slate-50/50 rounded-2xl text-lg font-bold p-5 focus:ring-orange-500 focus:border-orange-500 transition-all" :value="old('title', $aboutRole->title)" required />
                    ...
                    <div class="space-y-4">
                        ...
                        <textarea id="description" name="description" rows="5" class="block w-full border-slate-100 bg-slate-50/50 rounded-3xl text-sm font-medium p-6 focus:ring-orange-500 focus:border-orange-500 transition-all" required>{{ old('description', $aboutRole->description) }}</textarea>
                    ...
                    <div class="space-y-6">
                        ...
                        <div id="key-areas-container" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($aboutRole->key_areas as $area)
                                <div class="relative">
                                    <x-text-input name="key_areas[]" type="text" class="block w-full border-slate-100 bg-slate-50/50 rounded-2xl text-xs font-bold p-4 opacity-80" :value="$area" />
                        ...
                        <p class="text-[10px] text-slate-400 italic">Modify or add focus areas for this professional profile.</p>
                    </div>
                </div>
